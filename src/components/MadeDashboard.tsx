...
import { ProjectModePicker } from './LeftSidebar.js';
...
/**
 * The Made dashboards — Execute (`/execute`), Vibe (`/vibe`), Demo (`/demo`): the nav-reorg
 * split the former one `/make` command surface into THREE, one per fork of the old Make union
 * (build → Execute, document → Vibe, video → Demo). This is ONE parameterized component (the
 * `TestingPage` page-prop shape), not three copies:
 *
 *  - **Execute** = the RUN half — a KPI band (runs consumed w/ window delta + sparkline, active,
 *    needs-you, failed w/ delta), a first-class FilterStrip, and the run-card grid (needs-you
 *    floats first and jumps to the gate; failed rows carry an inline Retry — a prefill, never a
 *    hidden relaunch). NO docs grid. The header's ＋ launches a build run directly.
 *  - **Vibe** = the DOCUMENT corpus (docs whose kind is not `demo`), with the explicit doc
 *    fan-out and the EC24 corpus label. The header's ＋ opens a project-picker locked to Document.
...
type RunChip = 'all' | 'needs-you' | 'active' | 'failed' | 'done';

function matchesRunChip(status: string, chip: RunChip): boolean {
...
function ExecuteDashboard({ runs, navigate, runPath }: {
  runs: SessionView[];
  navigate: (path: string) => void;
  runPath: (id: string) => string;
}): React.ReactElement {
  ...
  // The spine: non-chat (build) runs — complete, all projects, needs-you first.
  const made = useMemo(
  ...
  const visibleRuns = searchedRuns.filter((v) => matchesRunChip(v.session.status, chip));
  const hiddenByWindow = q === '' ? made.length - buckets.current.length : 0;

  const chipCounts = useMemo(() => {
    const counts: Record<RunChip, number> = { all: 0, 'needs-you': 0, active: 0, failed: 0, done: 0 };
    for (const v of searchedRuns) {
      counts.all += 1;
      for (const c of ['needs-you', 'active', 'failed', 'done'] as const) {
        if (matchesRunChip(v.session.status, c)) counts[c] += 1;
      }
    }
    return counts;
  }, [searchedRuns]);
    ...
    <div className="flex flex-col" style={{ color: 'var(--ink-high)', padding: '0 var(--space-8) var(--space-8)', gap: 'var(--space-4)' }}>
      <div className="pt-8 flex items-center justify-between gap-4">
        <div className="flex items-baseline gap-4 min-w-0">
          <h1 className="text-2xl font-semibold font-mono" style={{ margin: 0 }}>Execute</h1>
      ...
      <KpiBand testId="execute-kpis">
        ...
        <KpiGroup label="Pipeline" grow={2}>
          ...
          <StatTile
            ...
            context={liveCounts.gates > 0 ? `${liveCounts.gates} waiting on you` : 'nothing waiting'}
      ...
      <FilterStrip
        testId="execute-filter"
        query={query}
        onQuery={setQuery}
        placeholder="Search runs…"
        chips={chips}
        active={chip}
        onChip={(id) => setChip(id as RunChip)}
        range={range}
        onRange={setRange}
      >
        {hiddenByWindow > 0 && (
          <button
            type="button"
            data-testid="execute-hidden-chip"
      ...
      <div data-testid="execute-list" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        {made.length === 0 && (
          <p style={{ margin: 0, fontSize: 'var(--text-xs)', color: 'var(--ink-dim)', fontFamily: 'var(--font-sans)', fontStyle: 'italic' }}>
            Nothing run yet — ＋ Run launches a governed build.
          </p>
        )}
        {made.length > 0 && visibleRuns.length === 0 && (
          <p data-testid="execute-runs-empty" style={{ margin: 0, fontSize: 'var(--text-xs)', color: 'var(--ink-dim)', fontStyle: 'italic' }}>
            No runs match this filter{hiddenByWindow > 0 ? ` — ${hiddenByWindow} sit outside the ${rangeWord(range)} window` : ''}.{' '}
            <button
              type="button"
              data-testid="execute-clear-filters"
        ...
        {visibleRuns.length > 0 && (
          <DashboardGrid testId="execute-runs-grid" min={340}>
...
// ── Vibe / Demo — the document / demo corpus ────────────────────────────────────

const VIBE_COPY = {
  vibe: {
    title: 'Vibe',
    subtitle: 'documents · decks · reports',
    createLabel: '＋ Document',
    glyph: '▤',
    empty: 'No documents yet — ＋ Document opens the interactive canvas in a project.',
  },
  demo: {
    title: 'Demo',
    subtitle: 'demo storyboards',
    createLabel: '＋ Demo',
    glyph: '▶',
    empty: 'No demos yet — ＋ Demo opens the storyboard in a project.',
  },
};
...
function CorpusDashboard({ mode, navigate }: {
  ...
  return (
    <div className="flex flex-col" style={{ color: 'var(--ink-high)', padding: '0 var(--space-8) var(--space-8)', gap: 'var(--space-4)' }}>
      <div className="pt-8 flex items-center justify-between gap-4">
        <div className="flex items-baseline gap-4 min-w-0">
          <h1 className="text-2xl font-semibold font-mono" style={{ margin: 0 }}>{copy.title}</h1>
          <p style={{ margin: 0, fontSize: 'var(--text-2xs)', color: 'var(--ink-dim)', fontFamily: 'var(--font-sans)' }}>
            {copy.subtitle}
          </p>
        </div>
      ...
      <KpiBand testId={`${mode}-kpis`}>
        <KpiGroup label="Corpus" grow={1}>
          <StatTile
      ...
      <div ref={whyRef} className="relative flex items-center gap-3 flex-wrap">
        ...
        {whyOpen && (
          <p
            data-testid={`${mode}-corpus-why-popover`}
      ...
      <div data-testid={`${mode}-list`} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        {docRows.length === 0 && (
          <p style={{ margin: 0, fontSize: 'var(--text-xs)', color: 'var(--ink-dim)', fontFamily: 'var(--font-sans)', fontStyle: 'italic' }}>
            {copy.empty}
          </p>
